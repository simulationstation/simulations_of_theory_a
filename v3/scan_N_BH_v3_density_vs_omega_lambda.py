#!/usr/bin/env python3
"""
scan_N_BH_v3_density_vs_omega_lambda.py

Driver script to scan N_BH_v3_density(Omega_Lambda) - the BH number density
per comoving volume, integrated over cosmic time only (no dVc/dz factor).

Outputs:
- CSV file: N_BH_v3_density_vs_omega_lambda.csv
- PNG plot: N_BH_v3_density_vs_omega_lambda.png
- Sample table to stdout

Usage:
    python3 scan_N_BH_v3_density_vs_omega_lambda.py
"""
...
from N_BH_v3_press_schechter_sfr import N_BH_v3_density


def scan_N_BH_v3_density(omega_min=0.01, omega_max=0.95, n_points=35,
                          z_max=10.0, nz=300):
    """
    Scan N_BH_v3_density over a range of Omega_Lambda values.
    ...
    omega_array : ndarray
        Omega_Lambda values
    N_BH_raw : ndarray
        Unnormalized N_BH_density values
    N_BH_norm : ndarray
        Normalized N_BH_density values (max = 1)
    """
    ...
    for i, ol in enumerate(omega_array):
        ...
        N_BH_raw[i] = N_BH_v3_density(ol, z_max=z_max, nz=nz)
...
def save_csv(omega_array, N_BH_raw, N_BH_norm,
             filename="N_BH_v3_density_vs_omega_lambda.csv"):
    """
    ...
    omega_array : ndarray
        Omega_Lambda values
    N_BH_raw : ndarray
        Raw N_BH_density values
    N_BH_norm : ndarray
        Normalized N_BH_density values
    filename : str
        Output filename
    """
    ...
    with open(filepath, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['Omega_Lambda', 'N_BH_v3_density_raw', 'N_BH_v3_density_norm'])
...
def create_plot(omega_array, N_BH_norm, filename="N_BH_v3_density_vs_omega_lambda.png"):
    """
    Create and save plot of N_BH_v3_density_norm vs Omega_Lambda.
    ...
    ax.annotate(f'Peak\n$\\Omega_\\Lambda = {omega_peak:.3f}$',
                ...
                xytext=(omega_peak + 0.15, N_peak - 0.15),
    ...
    ax.set_title('v3 Model: BH Number Density (per comoving volume) vs $\\Omega_\\Lambda$\n'
                 '(Time integral only, no volume factor)',
    ...
    ax.text(0.02, 0.02,
            ...
            '\\left|\\frac{dt}{dz}\\right|$ (no $dV_c/dz$)',
...
def print_sample_table(omega_array, N_BH_raw, N_BH_norm):
    ...
    print("\n" + "=" * 60)
    print("SAMPLE TABLE: N_BH_v3_density vs Omega_Lambda")
    print("=" * 60)
    ...
    print(f"\nPeak: Omega_Lambda = {omega_array[idx_peak]:.4f}, "
          f"n_BH_v3_density_norm = {N_BH_norm[idx_peak]:.4f}")

    # Our universe
    idx_07 = np.argmin(np.abs(omega_array - 0.7))
    print(f"Our Universe (Omega_Lambda ~ 0.7): n_BH_v3_density_norm = {N_BH_norm[idx_07]:.4f}")
...
def main():
    ...
    print()
    print("=" * 70)
    print("v3 MODEL: n_BH_density(Omega_Lambda) - Per Comoving Volume")
    print("=" * 70)
    print()
    ...
    print("  - BH rate: n_dot_BH ~ SFR0 * F_coll")
    print("  - TIME INTEGRAL ONLY: n_BH ~ integral n_dot_BH * |dt/dz| dz")
    print("  - NO volume factor dVc/dz (unlike N_BH_v3)")
    print()

    # Run scan
    omega_array, N_BH_raw, N_BH_norm = scan_N_BH_v3_density(
        omega_min=0.01,
        omega_max=0.95,
        n_points=35,
        z_max=10.0,
    ...
    print("=" * 70)
    print(f"""
The density version integrates BH formation over time only:

n_BH(OL) ~ integral dz * SFR0(z) * F_coll(z,OL) * |dt/dz|

This removes the comoving volume factor dVc/dz that caused N_BH_v3
to increase with Omega_Lambda (larger cosmological constant = more volume).

KEY DIFFERENCE FROM N_BH_v3:
- N_BH_v3: Total BH count in spacetime volume (includes dVc/dz)
- n_BH_density: BH density per comoving volume (time integral only)
